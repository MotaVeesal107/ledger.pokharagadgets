...
$sql = "SELECT pu.id, pu.purchase_date, pu.bill_ref, pa.name AS supplier_name,
               p.name AS product_name, p.device_model, pi.qty, pi.cost_price, pi.line_total, pi.serial_no
...
if (($_GET['export'] ?? '') === 'csv') {
    $csvRows = array_map(fn($r) => [$r['purchase_date'], $r['bill_ref'], $r['supplier_name'], $r['product_name'], $r['device_model'], $r['qty'], $r['cost_price'], $r['line_total'], $r['serial_no']], $rows);
    export_csv('purchases_report.csv', ['Date', 'Bill Ref', 'Supplier', 'Product', 'Device Model', 'Qty', 'Cost Price', 'Line Total', 'Serial'], $csvRows);
}
...
<div class="card">
  <table class="table mb-0">
    <thead><tr><th>Date</th><th>Bill</th><th>Supplier</th><th>Product</th><th>Device Model</th><th>Qty</th><th class="text-end">Cost</th><th class="text-end">Line Total</th><th>Serial</th></tr></thead>
    <tbody>
